...
const getCommandeById = async (req, res) => {
  try {
    const commande = await Car.findById(req.params.id).populate(
      "produits.prod_id user_id"
    );
    if (!commande) {
...
const getCarByMyId = async (req, res) => {
  try {
    const commande = await Car.find({ user_id: req.route.meta.user._id }).sort({
      createdAt: -1,
    });
    if (!commande) {
      return res.status(404).json({ error: "Commande non trouvée" });
...
const getCommandeByUserId = async (req, res) => {
  try {
    const commande = await Car.find({ user_id: req.params.id }).populate(
      "produits.prod_id"
    );
    if (!commande) {
      return res.status(404).json({ error: "Commande non trouvée" });
...
const updateCar = async (req, res) => {
  try {
    const { immat, marque, model, fab, kilo, _id } = req.body;
    ...
    if (!car) {
      return res.status(404).json({ error: "Car non trouvée" });
    }
...
const deleteCar = async (req, res) => {
  try {
    ...
    if (!car) {
      return res.status(404).json({ error: "Car trouvée" });
    }
    res.status(200).json({ message: "Car supprimée avec succès" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
  ...
  try {
    ...
    if (type === "Livré")
      id_prods.map(async (prod) => {
        ...
        if (prods.quantity == 0) prods.statusProd = "épuisé";
